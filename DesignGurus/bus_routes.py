...
class Solution:
    def numBusesToDestination(self, routes: list[list[int]], source: int, target: int) -> int:
        ...
        for bus, route in enumerate(routes):
            for stop in route:
                stops_to_buses[stop].append(bus)  # Fixed: Changed .append[bus] to .append(bus)
        
        # Initialize BFS
        queue = deque()
        stops_visited = set()  # Fixed: Changed tuple () to set()
        used_buses = set()

        # Start BFS from source stop
        queue.append((source, 0))  # Fixed: Changed to tuple (source, 0)
        ...
        while queue:
            ...
            for bus in stops_to_buses[current_stop]:
                ...
                for next_stop in routes[bus]:  # Fixed: Changed route[bus] to routes[bus]
                    if next_stop == target:
                        return buses_taken + 1
                    if next_stop not in stops_visited:
                        queue.append((next_stop, buses_taken+1))  # Fixed: Changed to tuple
